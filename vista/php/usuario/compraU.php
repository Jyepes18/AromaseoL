...
<body onload="calcularTotal();">
    ...
    <div class="d-flex justify-content-center">
        <div class="form">
            <form action="../../../modelo/usuarios/compra/añadir.php" method="post" onsubmit="return validar();">
                ...
                <div class="form-floating mb-3">
                    <input type="text" class="form-control" id="precio" name="precio" placeholder="Precio" autocomplete="cc-number" value="$<?php echo number_format($precio, 0, ',', '.'); ?>" onchange="calcularTotal()" />
                    <label for="precio">Precio</label>
                </div>

                <div class="form-floating mb-3">
                    <input type="number" class="form-control" id="cantidad" name="cantidad" placeholder="Cantidad" autocomplete="name" value="1" onchange="calcularTotal()" oninput="calcularTotal()" />
                    <label for="cantidad">Cantidad</label>
                </div>

                <div class="form-floating mb-3">
                    <input type="number" class="form-control" id="total" name="total" placeholder="Total" autocomplete="cc-number" readonly onchange="calcularTotal()" />
    ...
    <script>
        ...
        function calcularTotal() {
            var precio = parseFloat(document.getElementById("precio").value.replace("$", "").replace(".", "").replace(",", "."));
            var cantidad = parseInt(document.getElementById("cantidad").value);
            var total = precio * cantidad;

            // Formatear el total con un punto cada tres dígitos
